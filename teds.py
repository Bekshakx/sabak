class Nalog:
    ...
    def __init__(self):
        ...
        for i in range(len(s)):
            ...
            ms=int(s[i])*0.015
            Nalog.y=int(s[i])-(opv+ipn)
            nald=so+sn+ms
            print("\nҚызметкер таза жалақысы: {}\t опв={}\t ипн={}\t со={}\t сн={}\t мед.стр={}\t Компания налогы: {}\n ".format(int(Nalog.y),int(opv),int(ipn),int(so),int(sn),int(ms),int(nald)))
            nall.append(int(nald))
            tabis.append(int(Nalog.y))
        for item1 in range(len(nall)):
            Nalog.nall1+=int(nall[item1])
        for item in range(len(tabis)):
            Nalog.mem+=int(tabis[item])
        print("\nҚызметкерлерге берілетін жалақы: {}\t|  Компания жалпы налогы: {}\n ".format(Nalog.mem,Nalog.nall1))
    def taza(self):
    ...
    def method(task):
        ...
        nal=t*0.03
        nal1=(t-sh)*0.1
        jsh=(nal+nal1+Nalog.nall1+Nalog.mem)
        jt=t-jsh-sh
